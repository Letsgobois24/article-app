<x-dashboard.layout>
    <x-title>Create New Post</x-title>
    <form method="POST" action="/dashboard/posts" enctype="multipart/form-data"
        class="max-w-5xl grid md:grid-cols-2 gap-x-4 gap-y-1" x-data="{ title: '{{ old('title') }}', slug: '{{ old('slug') }}' }">
        @csrf
        {{-- Title --}}
        <div class="col-span-full">
            <label for="title" class="block text-sm/6 font-medium text-gray-900">Title</label>
            <input x-model="title" id="title" type="text" name="title" required autofocus
        ...
        <div>
            ...
            <div>
                <div class="flex">
                    <input x-model="slug" x-on:focus="slug == '' ? slug=slugify(title) : false" id="slug" required
                        type="text" name="slug"
                        class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6" />
        ...
        <div class="col-span-full">
            ...
            <input class="post-body" id="body" type="hidden" name="body" value="{{ old('body') }}">
        ...
        {{-- Submit --}}
        <button type="submit"
            class="cursor-pointer mt-4 text-white col-span-full bg-brand box-border border border-transparent hover:bg-brand-strong focus:ring-4 focus:ring-brand-medium shadow-xs font-medium leading-5 rounded-base text-sm px-4 py-2.5 focus:outline-none">New
            Post</button>
